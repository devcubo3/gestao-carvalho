...
interface DeleteDevelopmentDialogProps {
  open: boolean
...
export function DeleteDevelopmentDialog({
  open,
  onOpenChange,
  development,
  onConfirm,
  submitting = false,
}: DeleteDevelopmentDialogProps) {
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-destructive">
            ...
            Excluir Empreendimento Permanentemente
          </DialogTitle>
          <DialogDescription className="text-destructive">
            ⚠️ Esta ação NÃO pode ser desfeita! Os dados serão removidos permanentemente do banco de
            dados.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            <div className="rounded-lg border border-destructive bg-destructive/10 p-4">
              ...
              <div className="space-y-1 bg-background/50 p-3 rounded">
                ...
                <p className="text-sm text-muted-foreground">Nome: {development?.name}</p>
                <p className="text-sm text-muted-foreground">
                  Cidade: {development?.city} - {development?.state}
                </p>
                {development?.reference_value && (
                  <p className="text-sm text-muted-foreground">
                    Valor de Referência: {formatCurrency(development.reference_value)}
                  </p>
                )}
              </div>
            ...
            <div className="grid gap-2">
              <Label htmlFor="confirmText" className="text-sm">
                Para confirmar a <strong className="text-destructive">exclusão permanente</strong>,
